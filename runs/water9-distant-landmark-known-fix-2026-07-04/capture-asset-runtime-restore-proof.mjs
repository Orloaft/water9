...
const outDir = resolve('runs/water9-distant-landmark-known-fix-2026-07-04/asset-runtime-restore-proof');
const host = '127.0.0.1';
const sourceSelector = '#game canvas';
const viewport = { width: 1280, height: 800 };
const manifestPath = resolve('public/assets/generated/background-phase3/background-phase3.manifest.json');
...
async function choosePort() {
  const requested = Number(process.env.WATER9_ASSET_RUNTIME_RESTORE_PORT ?? 0);
  if (requested) {
...
function log(message) {
  console.error(`[asset-runtime-restore-proof] ${message}`);
}
...
async function readManifestAssets() {
  ...
  return manifest.assets
    .filter((asset) => requiredAssetIds.includes(asset.id))
    .map((asset) => ({
      id: asset.id,
      role: asset.role,
      band: asset.band,
      ...
      trimCrop: asset.trimCrop ?? null,
      size: asset.size,
      transparentPixels: asset.transparentPixels,
      semiTransparentPixels: asset.semiTransparentPixels,
      notes: asset.notes,
    }));
}

async function buildContactSheet(browser, captures, manifestAssets) {
  ...
  `))).join('');
  const manifestList = manifestAssets.map((asset) => `<li><b>${htmlEscape(asset.id)}</b>: ${htmlEscape(asset.path)} source=${htmlEscape(asset.sourcePath)} trimCrop=${htmlEscape(JSON.stringify(asset.trimCrop))}</li>`).join('');
...
<style>
  ...
  p, li { margin: 0 0 7px; color: #a9c4c9; }
  ...
  ul { margin: 0 0 20px; padding-left: 20px; font-size: 12px; }
</style>
<h1>Water9 Distant Landmark Asset Runtime Restore Proof</h1>
<ul>${manifestList}</ul>
<main>${cards}</main>`;
  const htmlPath = resolve(outDir, 'water9-distant-landmark-asset-runtime-restore-contact-sheet.html');
  const path = resolve(outDir, 'water9-distant-landmark-asset-runtime-restore-contact-sheet.png');
  await writeFile(htmlPath, html, 'utf8');
  const page = await browser.newPage({ viewport: { width: 1700, height: 2700 }, deviceScaleFactor: 1 });
...
try {
  ...
  for (const scenario of scenarios) {
    log(`scenario ${scenario.key} start`);
    ...
    const review = await command(page, 'backgroundReview', {
      label: `water9-distant-landmark-asset-runtime-restore-${scenario.key}`,
  ...
  const contactSheet = await buildContactSheet(browser, captures, manifestAssets);
  const gitStatusAfter = git(['status', '--short']);
  const notesPath = resolve(outDir, 'water9-distant-landmark-asset-runtime-restore-provenance.json');
  const notes = {
    schema: 'water9-distant-landmark-asset-runtime-restore-proof@1',
    ...
    commandsRun: [
      'npm run build',
      'node runs/water9-distant-landmark-known-fix-2026-07-04/capture-asset-runtime-restore-proof.mjs',
    ],
    manifestAssets,
    captures,
    contactSheet,
