...
const outDir = resolve('runs/water9-distant-landmark-known-fix-2026-07-04/local-micro-proof');
...
<main>${cards}</main>`;
  const htmlPath = resolve(outDir, 'water9-distant-landmark-local-micro-contact-sheet.html');
  const path = resolve(outDir, 'water9-distant-landmark-local-micro-contact-sheet.png');
...
try {
  ...
  const notesPath = resolve(outDir, 'water9-distant-landmark-local-micro-provenance.json');
  const notes = {
    schema: 'water9-distant-landmark-local-micro-proof@1',
