...
const outDir = resolve('runs/water9-distant-landmark-damage-control-2026-07-04/restore-proof');
...
<h1>Water9 Distant Landmark Restore Proof</h1>
<main>${cards}</main>`;
  const htmlPath = resolve(outDir, 'restore-proof-contact-sheet.html');
  const path = resolve(outDir, 'restore-proof-contact-sheet.png');
...
try {
  ...
  for (const target of targets) {
    ...
    const review = await command(page, 'backgroundReview', {
      label: `restore-proof-${target.label}`,
      depth: target.depth,
  ...
  const provenancePath = resolve(outDir, 'restore-proof-provenance.json');
  await writeFile(provenancePath, JSON.stringify({
    schema: 'water9-distant-landmark-restore-proof/v1',
