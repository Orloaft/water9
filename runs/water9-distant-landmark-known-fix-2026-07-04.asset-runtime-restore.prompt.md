# Water9 Distant Landmark Asset/Runtime Restoration
...
Goal:
Recover the distant-landmark known fix after three visual failures. This is not another opacity/blend tuning pass. Replace/reframe the bad source/runtime assets so normal gameplay canvas proof finally matches the subtle wide July 3 background read.
...
- `water9-biome-landmark-brine-shelf-gpt.png` may be closer painterly/horizontal source, but the current runtime crops/scales it into a narrow vertical-ish read. Inspect and reframe/crop/rebuild as needed.
...
Scope:
- Inspect the current asset files, source inbox files, build scripts, manifest entries, and render path involved in generated background/landmark loading.
...
Return block:
- Status: READY_FOR_MANAGER_VISUAL_REVIEW or BLOCKED.
- Changed files.
- Exact proof paths.
...
Remember: worker self-verdicts do not accept the visual. The manager will inspect the proof and decide.
