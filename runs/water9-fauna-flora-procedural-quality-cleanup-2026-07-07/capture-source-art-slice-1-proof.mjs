...
const faunaScenarios = [
  { key: 'abyss-fauna-viperfish', biome: 3, species: 'Abyssal Viperfish', assetKey: 'fauna-abyss-viperfish', distance: 48 },
  { key: 'abyss-fauna-lantern-swarm', biome: 3, species: 'Lantern Swarm', assetKey: 'fauna-abyss-lantern-swarm', distance: 58 },
  { key: 'hadal-fauna-static-fry', biome: 4, species: 'Static Fry', assetKey: 'fauna-abyss-static-fry', distance: 54 },
  { key: 'hadal-fauna-hatchet-school', biome: 4, species: 'Abyssal Hatchet School', assetKey: 'fauna-abyss-hatchet-school', distance: 58 },
  { key: 'hadal-fauna-microfish', biome: 4, species: 'Hadopelagic Microfish', assetKey: 'fauna-abyss-microfish', distance: 62 },
  { key: 'hadal-fauna-snipe-eel', biome: 4, species: 'Snipe Eel', assetKey: 'fauna-abyss-snipe-eel', distance: 48 },
];

const floraScenarios = [
  { key: 'special-flora-oxygen-kelp', biomes: [2, 3, 4, 1], species: 'Oxygen Bloom', assetKey: 'flora-oxygen-kelp' },
  { key: 'special-flora-oxygen-bulb', biomes: [2, 3, 4, 1], species: 'Oxygen Bloom', assetKey: 'flora-oxygen-bulb' },
  { key: 'special-flora-biolume-rock-0', biomes: [2, 3, 4, 1], species: 'Lumen Nodule', assetKey: 'biolume-rock-0' },
  { key: 'special-flora-biolume-rock-1', biomes: [2, 3, 4, 1], species: 'Lumen Nodule', assetKey: 'biolume-rock-1' },
  { key: 'special-flora-biolume-crystal', biomes: [2, 3, 4, 1], species: 'Lumen Nodule', assetKey: 'biolume-crystal' },
];
...
async function captureFlora(page, baseUrl, scenario) {
  ...
  if (!selected) throw new Error(`no flora candidate for ${scenario.species} ${scenario.assetKey}`);
  const teleport = await command(page, 'teleportToFlora', { index: selected.index });
  await command(page, 'clearProofOverlays');
  const snap = await snapshot(page);
  ...
  return writeCapture(page, { ...scenario, biome: selectedBiome }, target ? {
  ...
  } : null, { kind: 'flora', teleport, candidateIndex: selected.index });
}
...
try {
  ...
  await writeFile(resolve(outDir, 'source-art-slice-1-normal-play-proof.json'), `${JSON.stringify(report, null, 2)}\n`);
  console.log(JSON.stringify({ ok: report.ok, port, captures: captures.length, failures }, null, 2));
  ok = report.ok;
} catch (error) {
  errors.push({ type: 'exception', text: error?.stack ?? String(error) });
  await writeFile(resolve(outDir, 'source-art-slice-1-normal-play-proof.json'), `${JSON.stringify({ ok: false, port, errors, serverLogs: serverLogs.join('').slice(-4000) }, null, 2)}\n`);
