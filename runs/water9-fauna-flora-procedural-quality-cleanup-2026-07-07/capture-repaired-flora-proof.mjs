...
const scenarios = [
  { key: 'surface-flora-moon-sponge', biome: 1, species: 'Moon Sponge', assetKey: 'terrain-edge-flora-moon-sponge', band: 'surface' },
  ...
  { key: 'mid-flora-ember-bloom', biome: 2, species: 'Ember Bloom', assetKey: 'terrain-edge-flora-ember-bloom', band: 'mid' },
];
...
try {
  ...
  await writeFile(resolve(outDir, 'normal-play-repaired-flora-proof.json'), `${JSON.stringify(report, null, 2)}\n`);
  console.log(JSON.stringify({ ok: report.ok, port, captures: captures.length, failures }, null, 2));
  ok = report.ok;
} catch (error) {
  errors.push({ type: 'exception', text: error?.stack ?? String(error) });
  await writeFile(resolve(outDir, 'normal-play-repaired-flora-proof.json'), `${JSON.stringify({ ok: false, port, errors, serverLogs: serverLogs.join('').slice(-4000) }, null, 2)}\n`);
