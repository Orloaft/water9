...
const outDir = '/mnt/nxt-dev/water9/runs/water9-biome-landmark-transition-audit-2026-07-06/b2';
const host = '127.0.0.1';
const selector = '#game canvas';
const viewport = { width: 1280, height: 800 };
const preferredPort = Number(process.env.B2_AUDIT_PORT ?? 5180);

const scenarios = [
  { key: 'b1-lower-before-b2-depth1260', biome: 1, depth: 1260, note: 'B1 late-run lower/transition-deep baseline before travel to B2' },
  { key: 'b2-entry-surface-depth90', biome: 2, depth: 90, note: 'B2 first-water/surface entry immediately after biome switch' },
  { key: 'b2-upper-first-landmark-depth180', biome: 2, depth: 180, note: 'B2 upper band, first normal-play brine shelf eligibility' },
  { key: 'b2-mid-brine-shelf-depth760', biome: 2, depth: 760, note: 'B2 mid-band brine shelf normal gameplay' },
  { key: 'b2-lower-before-exit-depth1260', biome: 2, depth: 1260, note: 'B2 lower band before transitionDeep/deep exit' },
  { key: 'b2-transitiondeep-exit-depth1500', biome: 2, depth: 1500, note: 'B2 transitionDeep just before travel to B3' },
  { key: 'b3-entry-surface-depth90', biome: 3, depth: 90, note: 'B3 first-water/surface entry after leaving B2' },
];

const exactScenarios = [
  { key: 'exact-b1-lower-before-b2-depth1260', biome: 1, depth: 1260, reviewX: 3600, note: 'Exact-depth B1 late-run baseline; backgroundReview metadata/canvas, no water-window clearing' },
  { key: 'exact-b2-entry-surface-depth90', biome: 2, depth: 90, reviewX: 4200, note: 'Exact-depth B2 surface entry; backgroundReview metadata/canvas, no water-window clearing' },
  { key: 'exact-b2-upper-first-landmark-depth180', biome: 2, depth: 180, reviewX: 4200, note: 'Exact-depth B2 upper brine shelf; backgroundReview metadata/canvas, no water-window clearing' },
  { key: 'exact-b2-mid-brine-shelf-depth760', biome: 2, depth: 760, reviewX: 4700, note: 'Exact-depth B2 mid brine shelf; backgroundReview metadata/canvas, no water-window clearing' },
  { key: 'exact-b2-lower-before-exit-depth1260', biome: 2, depth: 1260, reviewX: 5200, note: 'Exact-depth B2 lower band before exit; backgroundReview metadata/canvas, no water-window clearing' },
  { key: 'exact-b2-transitiondeep-exit-depth1500', biome: 2, depth: 1500, reviewX: 5600, note: 'Exact-depth B2 transitionDeep exit; backgroundReview metadata/canvas, no water-window clearing' },
  { key: 'exact-b3-entry-surface-depth90', biome: 3, depth: 90, reviewX: 4200, note: 'Exact-depth B3 surface entry; backgroundReview metadata/canvas, no water-window clearing' },
];

function backgroundReviewDepthForStateDepth(targetDepthMeters) {
  return targetDepthMeters * 2 / 3;
}
...
try {
  ...
  for (const scenario of exactScenarios) {
    ...
    await page.goto(url, { waitUntil: 'networkidle', timeout: 30000 });
    await waitForPlaytest(page, scenario.biome);
    await command(page, scenario.biome, 'backgroundReview', {
      label: scenario.key,
      depth: backgroundReviewDepthForStateDepth(scenario.depth),
    ...
    const snapshot = await page.evaluate(() => window.__AQUA_PLAYTEST__?.snapshot?.() ?? null);
    const colorPath = resolve(outDir, `${scenario.key}-review-canvas.png`);
    const grayscalePath = resolve(outDir, `${scenario.key}-review-canvas-grayscale.png`);
    await writeFile(colorPath, await canvasPng(page, 'color'));
    await writeFile(grayscalePath, await canvasPng(page, 'grayscale'));
    exactOutputs.push({
      ...scenario,
      ...
      sourceSelector: selector,
      viewport,
      captureMethod: 'Playwright Chromium; ?playtest biome route; backgroundReview exact-depth positioning with clearWaterWindow:false; no gameplay code changes; direct #game canvas toDataURL. B2 review mode intentionally suppresses the lamp beam.',
      backgroundReviewDepthParam: backgroundReviewDepthForStateDepth(scenario.depth),
      colorPath,
      grayscalePath,
      colorBytes: await fileBytes(colorPath),
      grayscaleBytes: await fileBytes(grayscalePath),
...
const proofPath = resolve(outDir, 'b2-landmark-transition-proof.json');
await writeFile(proofPath, `${JSON.stringify({
  schema: 'water9/b2-landmark-transition-audit@1',
