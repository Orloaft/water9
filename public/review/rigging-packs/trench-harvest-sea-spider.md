# Rigging Focus Pack: Trench Harvest Sea Spider (trench-harvest-sea-spider)
...
Runtime creature: `trench-harvest-sea-spider`
...
```bash
npm run sandbox:preview -- --id trench-harvest-sea-spider --kind source --best --serve --open --visual
```
...
npm run articulated:extract-plan -- --plan tools/scratch/trench-harvest-sea-spider-starter-plan.json
```
...
npm run content:quick-review -- --id trench-harvest-sea-spider
