# Rigging Focus Pack: Razor Kelp Harp (razor-kelp-harp)
...
Runtime creature: `razor-kelp-harp`
...
```bash
npm run sandbox:preview -- --id razor-kelp-harp --kind source --best --serve --open --visual
```
...
npm run articulated:extract-plan -- --plan tools/scratch/razor-kelp-harp-starter-plan.json
```
...
npm run content:quick-review -- --id razor-kelp-harp
