# Unmapped Prototype Packet: Velvet Lantern Cuttle (velvet-lantern-cuttle)
...
- Sandbox URL: `/?sandbox=velvet-lantern-cuttle`

```bash
npm run sandbox:preview -- --id velvet-lantern-cuttle --with diver --serve --open --visual
npm run sandbox:visual -- --ids velvet-lantern-cuttle --states idle,lunge,stunned --with diver
npm run content:acceptance-audit -- --id velvet-lantern-cuttle
```
