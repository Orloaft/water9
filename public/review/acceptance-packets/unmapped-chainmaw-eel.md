# Unmapped Prototype Packet: Chainmaw Eel (chainmaw-eel)
...
- Sandbox URL: `/?sandbox=chainmaw-eel`

```bash
npm run sandbox:preview -- --id chainmaw-eel --with diver --serve --open --visual
npm run sandbox:visual -- --ids chainmaw-eel --states idle,lunge,stunned --with diver
npm run content:acceptance-audit -- --id chainmaw-eel
```
