# Unmapped Prototype Packet: Sand Battery (sand-battery)
...
- Sandbox URL: `/?sandbox=sand-battery`

```bash
npm run sandbox:preview -- --id sand-battery --with diver --serve --open --visual
npm run sandbox:visual -- --ids sand-battery --states idle,lunge,stunned --with diver
npm run content:acceptance-audit -- --id sand-battery
```
