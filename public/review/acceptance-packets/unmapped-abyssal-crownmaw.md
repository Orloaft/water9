# Unmapped Prototype Packet: Abyssal Crownmaw (abyssal-crownmaw)
...
- Sandbox URL: `/?sandbox=abyssal-crownmaw`

```bash
npm run sandbox:preview -- --id abyssal-crownmaw --with diver --serve --open --visual
npm run sandbox:visual -- --ids abyssal-crownmaw --states idle,lunge,stunned --with diver
npm run content:acceptance-audit -- --id abyssal-crownmaw
```
