# Unmapped Prototype Packet: Harpoon Cone (harpoon-cone)
...
- Sandbox URL: `/?sandbox=harpoon-cone`

```bash
npm run sandbox:preview -- --id harpoon-cone --with diver --serve --open --visual
npm run sandbox:visual -- --ids harpoon-cone --states idle,lunge,stunned --with diver
npm run content:acceptance-audit -- --id harpoon-cone
```
