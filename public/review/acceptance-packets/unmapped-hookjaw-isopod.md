# Unmapped Prototype Packet: Hookjaw Isopod (hookjaw-isopod)
...
- Sandbox URL: `/?sandbox=hookjaw-isopod`

```bash
npm run sandbox:preview -- --id hookjaw-isopod --with diver --serve --open --visual
npm run sandbox:visual -- --ids hookjaw-isopod --states idle,lunge,stunned --with diver
npm run content:acceptance-audit -- --id hookjaw-isopod
```
