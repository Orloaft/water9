# Unmapped Prototype Packet: Sawback Ray (sawback-ray)
...
- Sandbox URL: `/?sandbox=sawback-ray`

```bash
npm run sandbox:preview -- --id sawback-ray --with diver --serve --open --visual
npm run sandbox:visual -- --ids sawback-ray --states idle,lunge,stunned --with diver
npm run content:acceptance-audit -- --id sawback-ray
```
