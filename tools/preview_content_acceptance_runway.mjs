...
let port = Number(args.get('port') ?? 5193);
const json = args.has('json');
const serve = args.has('serve');
const openBrowser = args.has('open');
const noBuild = args.has('no-build');
const explicitId = String(args.get('id') ?? args.get('candidate') ?? '').trim();
const runwayPath = resolve(String(args.get('runway') ?? 'public/review/content-acceptance-runway.json'));
...
function relativeUrlFor(id) {
  return `/review/content-acceptance-runway.html${id ? `#${encodeURIComponent(id)}` : ''}`;
}

function runBuild() {
  const result = spawnSync('npm', ['run', 'content:acceptance-runway'], {
    cwd: process.cwd(),
    encoding: 'utf8',
    stdio: 'pipe',
  ...
  if (result.status !== 0) {
    ...
    throw new Error(`Failed to build content acceptance runway${output ? `:\n${output}` : ''}`);
  }
}
...
if (runway.schema !== 'water9/content-acceptance-runway@1') {
  throw new Error(`Unexpected content acceptance runway schema ${runway.schema ?? 'missing'}`);
}

const items = Array.isArray(runway.items) ? runway.items : [];
const unmapped = Array.isArray(runway.unmappedPrototypeThreats) ? runway.unmappedPrototypeThreats : [];
const allTargets = [...items, ...unmapped];
const recommended = runway.recommendedByStage?.[runway.summary?.nextBottleneck] ?? Object.values(runway.recommendedByStage ?? {})[0] ?? items[0] ?? null;
const target = explicitId ? allTargets.find((item) => item.id === explicitId) : null;
if (explicitId && !target) throw new Error(`No acceptance runway target found for ${explicitId}`);
...
const payload = {
  ...
  selectedStage: target?.stage ?? target?.status ?? null,
  recommendedId: recommended?.id ?? null,
  recommendedStage: recommended?.stage ?? null,
  summary: runway.summary,
  commands: {
    rebuild: 'npm run content:acceptance-runway',
    validate: 'npm run content:acceptance-runway-check',
    previewRunway: 'npm run content:acceptance-runway:preview',
    previewRecommended: `npm run content:acceptance-runway:preview -- --id ${recommended?.id ?? '<candidate-id>'}`,
...
if (json) {
  ...
  console.log('Water 9 content acceptance runway');
  console.log(`URL: ${payload.url}`);
  console.log(`Candidates: ${payload.summary?.candidates ?? 0}`);
  console.log(`Accepted threats: ${payload.summary?.acceptedThreats ?? 0}/${payload.summary?.targetThreats ?? 20}`);
  console.log(`Recommended: ${payload.recommendedId ?? 'none'} ${payload.recommendedStage ?? ''}`.trim());
