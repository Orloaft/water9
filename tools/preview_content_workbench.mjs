...
let port = Number(args.get('port') ?? 5178);
const json = args.has('json');
const serve = args.has('serve');
const openBrowser = args.has('open');
const noBuild = args.has('no-build');
const workbenchPath = resolve(String(args.get('workbench') ?? 'public/review/content-workbench.json'));
const relativeUrl = '/review/content-workbench.html';
...
function runBuild() {
  const result = spawnSync('npm', ['run', 'content:workbench'], {
    cwd: process.cwd(),
    encoding: 'utf8',
    stdio: 'pipe',
  });
  if (result.status !== 0) {
    ...
    throw new Error(`Failed to build content workbench${output ? `:\n${output}` : ''}`);
  }
}

async function loadWorkbench() {
  return JSON.parse(await readFile(workbenchPath, 'utf8'));
}
...
if (workbench.schema !== 'water9/content-workbench@1') {
  throw new Error(`Unexpected content workbench schema ${workbench.schema ?? 'missing'}`);
}
...
const payload = {
  ...
  rebuildCommand: 'npm run content:workbench',
  validateCommand: 'npm run content:workbench-check',
};
...
if (json) {
  ...
  console.log('Water 9 content workbench');
