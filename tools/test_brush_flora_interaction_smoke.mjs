...
const outDir = process.env.WATER9_BRUSH_FLORA_OUT_DIR ?? 'runs/water9-interactable-flora-stamps-2026-07-08';
const reportPath = process.env.WATER9_BRUSH_FLORA_REPORT ?? `${outDir}/brush-flora-smoke.json`;
const proofPath = process.env.WATER9_BRUSH_FLORA_PROOF ?? `${outDir}/brush-flora-proof.json`;
const host = '127.0.0.1';
const preferredPort = Number(process.env.WATER9_BRUSH_FLORA_PORT ?? 5181);

const cases = [
  { label: 'surface-brush', biome: 1, assetKeys: ['terrain-brush-flora-1', 'terrain-brush-flora-4', 'terrain-brush-flora-6', 'terrain-brush-flora-7'] },
  { label: 'mid-brush', biome: 2, assetKeys: ['terrain-brush-flora-1', 'terrain-brush-flora-4', 'terrain-brush-flora-6', 'terrain-brush-flora-7'] },
  { label: 'deep-brush', biome: 4, assetKeys: ['terrain-brush-flora-1', 'terrain-brush-flora-4', 'terrain-brush-flora-6', 'terrain-brush-flora-7'] },
];
...
async function stageAnyBrush(page, testCase, index = 0) {
  for (const assetKey of testCase.assetKeys) {
    const staged = await command(page, 'brushFloraSmokeStage', { assetKey, index });
    if (staged?.ok) return staged;
  }
  return await command(page, 'brushFloraSmokeStage', { index });
}
...
try {
  ...
  for (const testCase of cases) {
    ...
    const staged = await stageAnyBrush(page, testCase, 0);
    if (!staged?.ok) {
      fail(`${testCase.label} could not stage brush flora: ${JSON.stringify(staged)}`);
      continue;
    }
    await page.waitForTimeout(260);
    await capturePair(page, `${testCase.label}-before-interaction`);
    ...
    await capturePair(page, `${testCase.label}-after-scan`);
    ...
    await capturePair(page, `${testCase.label}-after-sample`);

    await command(page, 'brushFloraSmokeStage', { assetKey: staged.assetKey, species: staged.species, index: 0 });
    await command(page, 'selectTool', 'sonar');
    await holdKey(page, 'Space', 250);
    const afterSonar = await snapshot(page);
    const sonarContact = afterSonar?.state?.sonarContacts?.some((contact) => contact.kind === 'flora');
    if (!sonarContact) {
      fail(`${testCase.label} sonar did not expose a flora contact near ${staged.species}`);
    }
  ...
  await page.waitForTimeout(700);
  await command(page, 'clearProofOverlays');
  const supportStage = await stageAnyBrush(page, cases[0], 1);
  if (!supportStage?.ok) {
