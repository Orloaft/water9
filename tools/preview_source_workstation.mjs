...
let port = Number(args.get('port') ?? 5191);
...
const relativeUrl = '/review/source-candidates/source-workstation.html';
...
function runBuild() {
  const result = spawnSync('npm', ['run', 'source:workstation'], {
    cwd: process.cwd(),
    encoding: 'utf8',
    stdio: 'pipe',
  });
  if (result.status !== 0) {
    ...
    throw new Error(`Failed to build source workstation${output ? `:\n${output}` : ''}`);
  }
}

async function loadWorkstation() {
  return JSON.parse(await readFile(workstationPath, 'utf8'));
}
...
if (!noBuild) runBuild();
const workstation = await loadWorkstation();
if (workstation.schema !== 'water9/source-workstation@1') {
  throw new Error(`Unexpected source workstation schema ${workstation.schema ?? 'missing'}`);
}

const payload = {
  found: true,
  schema: workstation.schema,
  workstation: workstationPath,
  relativeUrl,
  url: absoluteUrl(),
  target: workstation.target?.id ?? null,
  species: workstation.target?.species ?? null,
  doctorStatus: workstation.target?.doctorStatus ?? null,
  rejections: workstation.rejections?.length ?? 0,
  devServerCommand: `npm run dev -- --port ${port}`,
  rebuildCommand: 'npm run source:workstation',
  validateCommand: 'npm run source:workstation-check',
};
...
if (json) {
  ...
  console.log('Water 9 source workstation');
  console.log(`URL: ${payload.url}`);
  console.log(`Target: ${payload.target} ${payload.species ?? ''}`.trim());
